...
{
	return NewObject<UTileEraseTool>(SceneState.ToolManager);
}

void UTileEraseTool::Setup()
{
	USingleClickTool::Setup();
	UE_LOG(LogTemp, Log, TEXT("TileEraseTool: Setup"));
}

FInputRayHit UTileEraseTool::IsHitByClick(const FInputDeviceRay& ClickPos)
{
	FIntPoint GridCoord;
...
void UTileEraseTool::OnClicked(const FInputDeviceRay& ClickPos)
{
	...
	if (HitTestGrid(ClickPos.WorldRay, GridCoord))
	{
		...
		{
			Sub->EraseAt(GridCoord);
		}
	}
}

bool UTileEraseTool::HitTestGrid(const FRay& WorldRay, FIntPoint& OutCoord) const
{
